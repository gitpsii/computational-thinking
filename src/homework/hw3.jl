...
begin
    double_letters = Char[]
    for i in 1:length(first_sample)-1
        if first_sample[i] == first_sample[i+1]
            push!(double_letters, first_sample[i])
        end
    end
end
...
# ╔═╡ b06a5c91-0ae5-4bed-afae-c13f983af4ff
sample_freq_matrix[23, :]   # everything following w
...
# ╔═╡ 5a8ffb27-bb4b-42e7-b3af-3a547f1eeb6f
sample_freq_matrix[:, 23]   # everything preceding w
...
begin
    ...
    for i in 1:27
        for j in 1:27
            row_sums[i] += sample_freq_matrix[i, j]
        end
    end
    row_sums
end
...
begin
    ...
    for j in 1:27
        for i in 1:27
            col_sums[j] += sample_freq_matrix[i, j]
        end
    end
    col_sums
end
...
begin
    ...
    for i in 1:27
        for j in 1:27
            row_col_answer += sample_freq_matrix[i, j]
        end
    end
    row_col_answer
end
...
manifest_format = "2.0"
project_hash = "0442d571b9d75c2439330068a8cd9495f743180c"
