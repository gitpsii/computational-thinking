...
# ╔═╡ 68114a4f-1ceb-4e13-a36c-fa7dd2db7d5c
# ╠═╡ disabled = true
#=╠═╡
begin
	v = [88,67,46]
...
# ╔═╡ 3a5a4b30-5d6b-4b48-bcd9-cbefb4cfe268
function get_greens(image::AbstractMatrix)
	return get_green.(image)
end

# ╔═╡ 915fd0d7-b124-405c-b63b-0577b60a7f83
function get_blue(pixel::AbstractRGB)
	return pixel.b
end

# ╔═╡ 52e5dd86-3816-4d4d-b268-c831c83857ae
...
function mean_color(image)
	r = 0
	for i in get_reds(image)
		r += i
	end
	mean_red = r/length(image)
	g = 0
	for j in get_greens(image)
		g += j
	end
	mean_green = g/length(image)
	b = 0
	for k in get_blues(image)
		b += k
	end
	mean_blue = b/length(image)
	avg_color = RGB(mean_red, mean_green, mean_blue)
	return avg_color
end
...
# ╔═╡ 571ad48a-7256-4534-8720-8a46b9b11c4c
length(philip)
...
function quantize(color::AbstractRGB)
	red = floor(10color.r)/10
	green = floor(10color.g)/10
	blue = floor(10color.b)/10
...
function noisify(color::AbstractRGB, s)
	color_red = color.r + (2rand() - 1) * s
	color_green = color.g + (2rand() - 1) * s
	color_blue = color.b + (2rand() - 1) * s
	return RGB(clamp(color_red, 0, 1), clamp(color_green, 0, 1), clamp(color_blue, 0, 1))
end
...
function noisify(image::AbstractMatrix, s)
	[noisify(color, s) for color in image]
end
...
The image is unrecognisable with intensity around 2.0
"""
...
manifest_format = "2.0"
project_hash = "44ff5ff6862c02928648b698f1b204204f7ea336"
